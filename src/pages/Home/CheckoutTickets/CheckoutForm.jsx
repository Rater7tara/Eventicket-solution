...
const CheckoutForm = ({
  ...
  console.log("🚀 Creating/updating payment intent for amount:", grandTotal);
  ...
  try {
    ...
    // UPDATED: Send ONLY the booking ID and coupon data to backend
    // Let the backend determine amounts and update existing payment intent
    const paymentData = {
      bookingId: currentBookingId,
      
      // Send coupon information so backend can update payment intent
      ...(appliedCoupon && {
        amount: grandTotal,
    ...
    // Call payment API - this will either update existing or create new payment intent
    ...
    // Handle response
    if (!response.ok) {
      if (responseData.code === 'PAYMENT_INTENT_LOCKED') {
        // Payment intent was already confirmed, need to refresh
    ...
    if (responseData.success) {
      ...
      if (responseData.updated) {
        console.log("✅ Payment intent updated successfully for amount:", grandTotal);
      } else {
