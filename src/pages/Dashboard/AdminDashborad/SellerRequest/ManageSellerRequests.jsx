...
const ManageSellerRequests = ({ onBack }) => {
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          "Failed to fetch seller requests. Check your admin privileges."
      );
      setLoading(false);
    }
  };
  ...
  const handleApproveSeller = async (sellerId) => {
    try {
      ...
      setSellerRequests(
        sellerRequests.filter((seller) => seller._id !== sellerId)
      );
  ...
  const handleMonitorSeller = async (sellerId) => {
    try {
      ...
      setError(
        err.response?.data?.message ||
          "Failed to update seller monitoring status"
      );
    }
  };
          ...
          <h3 className="text-xl font-semibold mb-4 text-gray-800 flex items-center">
            ...
            <span className="ml-auto text-sm bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full">
              {selectedSeller.status?.charAt(0).toUpperCase() +
                selectedSeller.status?.slice(1) || "Pending"}
            </span>
          </h3>
              ...
              <div className="space-y-2">
                <div className="flex">
                  ...
                  <span className="font-medium text-gray-800">
                    {selectedSeller.name || "Not provided"}
                  </span>
                </div>
                <div className="flex">
            ...
            <div className="bg-gray-50 p-4 rounded-md">
              ...
              <div className="space-y-2">
                <div className="flex">
                  ...
                  <span className="font-medium text-gray-800">
                    {selectedSeller.shopName || "Not provided"}
                  </span>
                </div>
                <div className="flex">
                ...
                <div className="flex">
                  ...
                  <span className="font-medium text-gray-800">
                    {selectedSeller.contactNumber || "Not provided"}
                  </span>
                </div>
                <div className="flex">
                ...
                <span
                  className={`font-medium px-2 py-1 rounded text-xs ${
                    selectedSeller.status === "pending"
                      ...
                      : selectedSeller.status === "denied"
                      ? "bg-red-100 text-red-800"
                      : "bg-gray-100 text-gray-800"
                  }`}
                >
                  {selectedSeller.status?.charAt(0).toUpperCase() +
                    selectedSeller.status?.slice(1) || "Unknown"}
                </span>
              </div>
              <div className="flex">
                ...
                <span className="font-medium text-gray-800">
                  {selectedSeller.updatedAt
                    ? new Date(selectedSeller.updatedAt).toLocaleDateString()
                    : "Not available"}
                </span>
              </div>
            </div>
      ...
      <div className="fixed inset-0 bg-gray-900/70 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity duration-300 ease-in-out">
        <div className="bg-white rounded-lg p-6 w-96 shadow-2xl transform transition-all duration-300 ease-out animate-fade-in-up">
          ...
          <p className="mb-6 text-gray-600">
            Are you sure you want to delete the seller request from{" "}
            <span className="font-medium text-gray-800">
              {selectedSeller.name}
            </span>
          ...
          {user?.role !== "admin" && (
            <div className="p-6 bg-yellow-50 border-b border-yellow-200">
              <div className="flex items-center text-yellow-700">
                ...
                <p>
                  You need admin privileges to manage seller requests. Current
