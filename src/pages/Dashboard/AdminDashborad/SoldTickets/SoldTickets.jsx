...
const SoldTickets = () => {
  ...
  const getAuthHeaders = () => {
    ...
    return {
      headers: {
        ...
        // Add these headers if your backend requires them
  ...
  const filteredTickets = tickets.filter((ticket) => {
    ...
    // Since paymentStatus might not exist, we'll check booking status or default to 'paid'
    const status = ticket.paymentStatus || ticket.status || "paid";
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <p className="text-sm text-gray-600 flex items-center">
                ...
                User ID:{" "}
                <span className="font-medium ml-1 text-gray-700">
                  {selectedTicket.buyerId?._id || "N/A"}
                </span>
              </p>
            </div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <p className="text-sm text-gray-600 flex items-center">
                ...
                <span className="font-medium ml-1 text-green-700">
                  {formatCurrency(
                    selectedTicket.totalAmount || selectedTicket.amount || 0
                  )}
                </span>
              </p>
            </div>
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <div className="p-6 border-b border-gray-200">
            ...
            <p className="text-gray-600 mt-1">
              View and manage all ticket sales in the system
            </p>
          </div>
          ...
          {user?.role !== "admin" && (
            <div className="p-6 bg-yellow-50 border-b border-yellow-200">
              <div className="flex items-center text-yellow-700">
                ...
                <p>
                  You need admin privileges to access this page. Current role:{" "}
                  {user?.role || "unknown"}
                </p>
              </div>
            </div>
                  ...
                  {currentTickets.map((ticket) => (
                    <tr key={ticket._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">
                        {formatCurrency(
                          ticket.totalAmount || ticket.amount || 0
                        )}
                      </td>
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-gray-800 mt-1">
                  {formatCurrency(
                    tickets.reduce(
                      (sum, ticket) =>
                        sum + (ticket.totalAmount || ticket.amount || 0),
                      0
                    )
                  )}
                </h3>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-bold text-gray-800 mt-1">
                  {tickets.reduce(
                    (sum, ticket) => sum + (ticket.seats?.length || 0),
                    0
                  )}
                </h3>
              </div>
